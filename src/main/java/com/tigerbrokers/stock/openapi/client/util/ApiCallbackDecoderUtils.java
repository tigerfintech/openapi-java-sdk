              ...
              if (null != arrayValue && arrayValue.length >= 2){
                ...
                if (serverSendInterval > 0 || serverReceiveIterval > 0){
                  if (null == ctx.channel().pipeline().get(IDLE_STATE_HANDLER)){
                    serverSendInterval = serverSendInterval == 0 ? 0 : serverSendInterval + WebSocketHandler.HEART_BEAT_SPAN;
                    serverReceiveIterval = serverReceiveIterval == 0? 0: serverReceiveIterval - WebSocketHandler.HEART_BEAT_SPAN;
